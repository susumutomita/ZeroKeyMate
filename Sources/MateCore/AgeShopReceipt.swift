...
public enum AgeShopReceipt {
    ...
    public static func validate(order: AgeShopOrder, logs: [Log]) throws {
        ...
        guard order.chainId == AgeShopProtocol.chainID, order.token.lowercased() == AgeShopProtocol.token else { throw AgeShopError.invalidPayment }
        ...
        let amount = CanonicalBytes.hexString(Data(repeating: 0, count: 24) + CanonicalBytes.u64(selection.amount))
