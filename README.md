...
| On-device conversation | [Build 18 continuity fixes and reproducible Mac evaluation](docs/conversation-quality.md): typed dialogue history, literal name/preferences, purchase-request recall and language continuity; physical update deferred. [Build 15 physical evidence](docs/conversation-device-status.md): English/Japanese switching, seven-turn recall and tested topic changes passed on iOS 26.7. Prepared first-text latency was 0.49–0.78 s across six fixed-text turns; microphone-to-speaker latency is not claimed. [New live-speech path and iOS 27 compatibility](docs/conversation-latency.md); the OS supplies the LLM. |
| Weather conversation | [Build 19](docs/weather-conversation.md): current forecasts from Open-Meteo, city clarification, dated answers and source links; unavailable data is never replaced with an invented forecast. Optional general search is described below. |
| Web search | [Build 20](docs/web-search.md): opt-in Brave retrieval, isolated on-device summaries, source links and honest unavailable replies. Requires a user-provided API key; authenticated provider and physical voice acceptance remain unverified. |
