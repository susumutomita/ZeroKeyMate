...
| On-device conversation | [Build 18 continuity fixes and reproducible Mac evaluation](docs/conversation-quality.md): typed dialogue history, purchase-request recall and language continuity; physical update deferred. [Build 15 physical evidence](docs/conversation-device-status.md): English/Japanese switching, seven-turn recall and tested topic changes passed on iOS 26.7. Prepared first-text latency was 0.49–0.78 s across six fixed-text turns; microphone-to-speaker latency is not claimed. [New live-speech path and iOS 27 compatibility](docs/conversation-latency.md); the OS supplies the LLM. |
