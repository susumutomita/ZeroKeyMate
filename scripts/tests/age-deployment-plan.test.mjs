...
function fixture() {
 const state={nonce:7,pending:7,chain:5042002,balance:0n,price:21000000000n,estimate:100000n,estimates:[],hash,time:now,code:undefined,occupied:false};
 const rpc={
...
test('unsigned Arc plan binds both CREATE addresses and constructor to consecutive deployer nonces',async()=>{
 ...
 const plan=await prepareAgeDeploymentPlan(pkg,deployer,[a.rpc,b.rpc],now);
 assert.equal(plan.verifier,getContractAddress({from:deployer,nonce:7n}).toLowerCase());
 assert.equal(plan.gate,getContractAddress({from:deployer,nonce:8n}).toLowerCase());
 ...
 assert.deepEqual(plan.transactions.map(t=>[t.chainId,t.from,t.to,t.nonce,t.value,t.gas]),
  [[5042002,deployer,null,7,'0','144000'],[5042002,deployer,null,8,'0','144000']]);
 ...
 assert.equal(a.state.estimates[1].stateOverride[0].nonce,8);
...
test('RPC disagreement, pending nonce, wrong network, stale head and occupied addresses reject preparation',async()=>{
 for(const change of [{chain:84532},{nonce:8,pending:8},{pending:8},{hash:'0x'+'33'.repeat(32)},
  ...
  await assert.rejects(()=>prepareAgeDeploymentPlan(pkg,deployer,[a.rpc,b.rpc],now));
  assert.equal(a.state.estimates.length+b.state.estimates.length,0);
 }
 const a=fixture();await assert.rejects(()=>prepareAgeDeploymentPlan(pkg,deployer,[a.rpc,a.rpc],now));
 await assert.rejects(()=>prepareAgeDeploymentPlan(pkg,'0x'+'00'.repeat(20),[a.rpc,fixture().rpc],now));
});
test('fee spikes and the combined deployment budget cannot be overridden by an estimate',async()=>{
 ...
 await assert.rejects(()=>prepareAgeDeploymentPlan(pkg,deployer,[a.rpc,b.rpc],now),/fee exceeds/);
 a.state.price=21000000000n;a.state.estimate=2000000n;
 await assert.rejects(()=>prepareAgeDeploymentPlan(pkg,deployer,[a.rpc,b.rpc],now),/total 0.10/);
 a.state.estimate=-1n;await assert.rejects(()=>prepareAgeDeploymentPlan(pkg,deployer,[a.rpc,b.rpc],now),/Invalid deployment gas/);
 a.state.estimate=100000n;a.state.balance=DEPLOYMENT_BUDGET;b.state.balance=DEPLOYMENT_BUDGET;
 assert.equal((await prepareAgeDeploymentPlan(pkg,deployer,[a.rpc,b.rpc],now)).fundingRequired,'0');
});
test('nonce consumption during estimation invalidates both planned contract addresses',async()=>{
 ...
 await assert.rejects(()=>prepareAgeDeploymentPlan(pkg,deployer,[a.rpc,b.rpc],now),/nonce changed during/);
});
