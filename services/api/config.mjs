...
import {networkConfiguration,stateDirectory} from './networks.mjs';
...
export const providerSchema=z.object({id:z.string().max(100).regex(/^11155111:[0-9]+$/),service:z.number().int().min(0).max(1),
 price:uintString.refine(v=>/^[1-9][0-9]*$/.test(v)),owner:address,recipient:address,ensName:ensName.or(z.literal('')).default(''),
...
export function configuration(e=process.env){
 ...
 try{
  ...
  requireValue(parsed.attestorMode!=='circle' || !keys.some(k=>privateKeyToAccount(k).address.toLowerCase()===parsed.attestorAddress.toLowerCase()),'key_separation','The Circle attestor must be separate from the relayer.',503);
  return {...parsed,providers,chainId:network.chainId,token:network.token,
   dataDirectory:stateDirectory('api',e),
