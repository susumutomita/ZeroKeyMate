...
## Companion gaze and physical ZK — 2026-09-08

The resumed implementation focuses on the companion: tapping the face opens a separate controls panel whose **Start companion** action starts camera tracking and opt-in continuous on-device voice. Docking or launching still does not start sensors. Face detection runs separately from slower image classification, and gaze follows both horizontal and vertical face position. Capture buffers are explicitly mirrored and rotated using AVFoundation's rotation coordinator before Vision consumes them. Reduce Motion removes interpolation/blinking, while preserving a static gaze indicating the detected position.

DockKit system-tracking requests are retried after failure instead of caching failure as success. Settings distinguishes absent hardware, a disabled stand tracking button, enabled tracking with no subject, and subjects reported by DockKit itself. A successful API call is not evidence that a physical motor followed a person. User confirmation of gaze/stand motion is pending.

Voice turns end after 15 seconds of initial silence, two seconds without a changed transcript, or 60 seconds total. A turn submits at most once. Approval/settings sheets, audio interruption, camera interruption, Rest, backgrounding and detach stop the voice session. Pending playback/resume callbacks cannot resume an ended session. Conversation still uses the original Apple Foundation Models path; shopping, age credentials and conversational rule setup are not implemented by this change.

Validation: `make test SWIFT_TEST_FLAGS=--disable-sandbox` passed (22 Swift, 25 Node, 6 launcher tests). Local HTTP tests required execution outside the managed listener restriction. `CFFIXED_USER_HOME="$PWD/.build/xcode-user" MATE_NESTED_SANDBOX=1 make build-ios` passed. The signed native app was installed and launched on an actual iPhone 16 Pro, iOS 26.6.1. A paired Wi-Fi phone initially had no active tunnel; the launcher now performs a bounded connection probe and rechecks the live state before installation, rather than treating an inactive tunnel as an absent phone.

Physical-device `NativeProofTests` passed **2 tests, 0 failures, 0 skips** in `.build/validation/physical-native-resume.xcresult`. The native production circuit generated a real proof and rejected a modified proof on that iPhone. Its exported XCTest proof attachment was independently accepted by the matching Mac ProveKit CLI verifier (exit 0). Proof SHA-256: `43fa97f2dbdcb3785222545e167930a67e9048453c8fbee0febe149939c8b9d0`. This was a developer test over Wi-Fi, not airplane-mode acceptance, a user share-sheet transfer, or a phone peak-memory measurement. Logs and proof are under `.build/validation/physical-native-*`; no actual user policy, camera frame or voice recording was exported.


The final home screen contains only the eyes. Visible headings, transcripts and persistent action buttons have moved to Controls, opened by tapping the face. Long-press rests the companion. VoiceOver retains the actual companion status and an explicit rest action. The missing asset catalog and AppIcon are now present, generated from Mate's existing paper/graphite eyes with a reproducible AppKit script. No sensor permission is implicitly granted by launching the icon.

Final face-only validation: `make build-ios` passed outside the managed CoreSimulator restriction (`.build/validation/face-only-ios.log`). `make start-device` installed and normally launched the signed app with AppIcon on the paired iPhone (`face-only-device.log`). Simulator UI acceptance passed six tests, including no visible home text/toolbar, portrait accessibility, landscape bounds, language persistence and proof preflight. The native proof UI test initially timed out with the untouched form still visible; waiting for the transitioned action to become hittable fixed the test, and its focused rerun passed (1 test, 0 failures, 0 skips; `face-proof-retry.xcresult`). This verifies native generation, tamper rejection and preparation of the shareable file in Simulator. No claim of a single all-green seven-test run or physical gaze/stand acceptance is made. Current face screenshots and their provenance are in `docs/assets`.

## Softer gaze — 2026-09-08

Replaced tall black eyes with smaller rounded eyes and a small highlight, reduced travel, removed the asymmetric thinking stare, softened blinking and changed Rest to curved closed eyes. No visible home text or controls were added. A pure `CompanionGaze` filter suppresses small detection jitter, limits speed, holds position for 0.6 seconds through brief face loss and then returns smoothly to center. Explicit capture stop resets the filter. Reduce Motion still disables visual animation. The face selector continues to choose the largest detected face; this does not implement identity tracking or a persistent multi-person target.

`make test SWIFT_TEST_FLAGS=--disable-sandbox` passed (27 Swift, 25 Node, 6 launcher tests), including five gaze jitter/loss/rate/reset cases. `make build-ios` and the signed generic-device build passed. The three selected face/portrait/landscape UI tests passed without failures or skips; screenshots were visually inspected. Logs and results are `.build/validation/gentle-gaze-*`. The first physical install attempt could not connect to the paired iPhone, so this revision has not yet replaced the app on that phone. Perceived comfort and physical tracking remain user acceptance, not assertions established by a screenshot or unit test.

## Restore the earlier eye design — 2026-09-08

The user preferred the earlier tall matte eyes to the smaller glossy design. Restored only `MateEyes` byte-for-byte from `598caff`, plus its existing unmodified screenshots. The tested `CompanionGaze` jitter/loss filter remains active in `MateModel`. `make test SWIFT_TEST_FLAGS=--disable-sandbox` passed (27 Swift, 25 Node, 6 launcher tests), and `make build-ios` passed (`.build/validation/restore-eyes-*`). No additional UI tests were added or run for this exact visual restoration. The rejected round-eye revision was never installed on the physical phone; no new physical installation was attempted for this restoration.
