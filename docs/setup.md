...
| 起動先 | コマンド | 実行内容 |
| --- | --- | --- |
| 選択メニュー | `make start` | `1`：Simulator、`2`：実機iPhone、`q`：キャンセル |
| 手元のiPhone | `make start-device` | 接続端末・Team選択 → ビルド・署名 → インストール → 起動 |
...
複数端末または複数Teamがある場合は、`MATE_DEVICE_UDID=UDID MATE_DEVELOPMENT_TEAM=TEAM_ID make start` で明示します。`make start-device` は選択メニューを省略して実機を起動します。`./mate --device auto` / `./mate --device UDID` / `./mate --simulator` も使用できます。
...
会話はFoundation Models、音声入力は英語（米国）の端末内Speech認識を使用します。対応していない場合はその理由を表示し、クラウドへ切り替えません。Simulatorではモデル・マイク・カメラを利用できない場合があります。外部の翻訳・要約は設定画面から直接依頼を作成でき、会話モデルによる分類に依存しません。
...
iPhoneから接続するときは、認証トークンを維持するHTTPSのリバースプロキシをAPIと専門サービスに用意してください。HTTPのLANアドレスは使用できません。APIも専門サービスも、既定では127.0.0.1だけで待ち受けます。接続設定は再ビルド不要です。Macで `npm run pair` を実行し、非公開ファイルの1回限りのコードをアプリの接続設定へ入力してください。コードは10分間、交換後の端末セッションは1時間または500リクエストまで有効です。アプリにはAPIトークンやサーバー用RPC資格情報を同梱しません。
...
月アイコンの **Rest** でカメラ・マイクと会話を停止します。カメラだけを止める場合は **Settings → Stop camera**。停止処理が終わり **Camera off** と表示されるまで待ってください。バックグラウンドへの移動・接続済みスタンドからの取り外しでも停止し、復帰や再装着では自動再開しません。Restは委任の失効や送信済み取引の取り消しではありません。

DockKit対応スタンドに装着しただけではアプリ起動・撮影・音声入力は始まりません。Mateを起動し **Settings → Start camera** を押すと、スタンドの接続と追尾ボタンが有効な場合にシステム追尾を要求します。実機での接続・追尾は未確認です。

起動コマンドはアプリを開いたら終了します。その後のCtrl+Cではアプリは停止しません。完全に閉じるにはiPhoneのアプリスイッチャー、SimulatorではCommand-Qを使います。

## 充電接続時にMateを開く

MateをインストールしたiPhoneで、ショートカット → オートメーション → ＋ → 充電器 → 接続されている、を選びます。「すぐに実行」を選択し、「アプリを開く」アクションで **Mate** を指定して保存します。確認スイッチを使う旧バージョンでは「実行の前に尋ねる」をオフにします。まずロックを解除した状態で充電器を接続し直して確認してください。iOSがロック解除を求める場合は解除が必要です。ロック中の無人起動は未検証です。

この設定はiPhone上で一度行う必要があります。Belkin以外の充電器にも反応し、カメラ・音声はアプリ内で明示的に開始します。自動起動を止めるには、このオートメーションを無効化または削除します。[Appleの充電器トリガー](https://support.apple.com/guide/shortcuts/apde31e9638b/ios)・[自動実行の設定](https://support.apple.com/guide/shortcuts/apd602971e63/ios)。
