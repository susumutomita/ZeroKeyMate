...
/// Native proving is isolated from UI and networking. No witness leaves this actor.
actor ProofService {
    ...
    func prepare() throws {
        ...
        guard manifest.system == "ProveKit",manifest.version == "1.0.1",manifest.circuit == "mate_policy",manifest.hash == "skyscraper" else {
        ...
        for (name,data) in [("mate_policy.pkp",prover),("mate_policy.pkv",verifier)] {
            guard LocalSecrets.hash(data).dropFirst(2) == manifest.files[name] else { throw ProductError.invalidResponse }
        }
