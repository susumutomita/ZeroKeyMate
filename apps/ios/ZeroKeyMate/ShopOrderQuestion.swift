...
enum ShopOrderQuestion {
    ...
    static func matches(_ input: String) -> Bool {
        ...
        let japanese = #"^(?:(?:mate|メイト)[、\s]*)?(?:(?:購入|注文|支払い|決済|年齢確認|年齢の証明)(?:は|が)?(?:もう)?(?:完了|終わっ|済ん|でき)(?:した|して|しまし|て|た|い|る|ます|ました|です|か|の|ね|よ|ん|まだ)*|(?:ビールは|ビールを|もう)?(?:買えた|買ってくれた|買ってきた)(?:の|か|です|ん|よ|ね)*)$"#
        let english = #"^(?:didyou(?:buy(?:my|the)?beer|(?:complete|finish)(?:my|the)?(?:order|purchase|payment))|haveyou(?:bought(?:my|the)?beer|(?:completed|finished)(?:my|the)?(?:order|purchase|payment))|is(?:my|the)?(?:order|purchase|payment|ageverification)(?:complete|completed|done|confirmed)|has(?:my|the)?(?:order|purchase|payment)(?:finished|completed)|(?:order|purchase|payment|ageverification)(?:complete|completed|done|status))(?:yet|already|now)?$"#
...
extension ShopCheckout.Phase {
    var purchaseAnswer: String {
        switch self {
        case .complete: return "Yes. Your Mate Lager test purchase is complete, and the payment is confirmed on Arc Testnet."
        ...
        case .paymentApproval: return "Not yet. Your age is verified. The purchase is waiting for you to approve 0.10 test USDC."
