        ...
        Form {
            ...
            if let evidence = model.evidence {
                Section("Verified on this device") {
                    ...
                    LabeledContent("Preparation", value: L10n.format("%lld ms", evidence.proof.preparationMilliseconds))
                    LabeledContent("Total proof request", value: L10n.format("%lld ms", evidence.proof.totalMilliseconds))
                    LabeledContent("Key setup", value: L10n.text(evidence.proof.usedCachedKeys ? "Cached in this session" : "Loaded for this request"))
